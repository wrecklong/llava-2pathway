...
#from transformers.utils import ALL_LAYERNORM_LAYERS
from typing import List, Optional

# try:
#     from llava.model.language_model.hybrid_decoder_layer_qwen3 import Qwen3RMSNorm
# except ImportError:
#     logger.warning("无法导入 Qwen3RMSNorm，使用占位类")
#     Qwen3RMSNorm = type("Qwen3RMSNorm", (nn.Module,), {})

# # 定义所有可能的 LayerNorm 层类型
# ALL_LAYERNORM_LAYERS = [
#     nn.LayerNorm,
#     Qwen3RMSNorm,
#     # 如果你还用 Llama/Mistral，可以加上
...
class LlavaTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            # ===== 调试打印：优化器创建前后统计 token_router 参数覆盖情况 =====
            # 解包真实模型，定位 token_router
            real_model = opt_model
            if hasattr(real_model, "module"):
                real_model = real_model.module
            for attr in ["base_model", "model", "module"]:
                if hasattr(real_model, attr):
                    maybe = getattr(real_model, attr)
                    if hasattr(maybe, "token_router"):
                        real_model = maybe
                        break
